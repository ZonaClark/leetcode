  ...
  if (!input.includes('+') && !input.includes('-') && !input.includes('*')) {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (isNaN(parseInt(char, 10))) {
        ...
        const leftParts = diff_ways_to_evaluate_expression(
          input.substring(0, i)
        );
        const rightParts = diff_ways_to_evaluate_expression(
          input.substring(i + 1)
        );
...
console.log(
  `Expression evaluations: ${diff_ways_to_evaluate_expression('2*3-4-5')}`
);
